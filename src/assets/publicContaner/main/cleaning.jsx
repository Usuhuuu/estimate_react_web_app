...
const Clean = () => {
  const [questionSets, setQuestionSets] = useState([
    [
      {id:6.1, text:'청소 종류 선택하세요', options:["이사 청소","입주 청소","에어컨 청소"]},
    ],
    [
      {id:10.1, text:'어떤 건물인지 선택해 주세요', options:["아파트","단독주택","빌라","오피스텔",'창고',"기타"]},
      {id:10.3, text:"시공 면적", options:['10평 이하','10평 - 20평', '30평 - 40평', '40평 - 50평', '해당 없음']},
      {id:10.7, text:"청소 회망 날자", options:['협의 가능해요','가능한 빨리 진행하고 싶습니다','일주일 이내로 진행하고 싶습니다','원하는 날짜가 있습니다','기타']},
    ...
    [
      {id:6.4, text:"에어컨 종류", options:['벽걸이에어컨','스텐드에어컨','시스템형(천장형 에어컨)']},
      {id:10.7, text:"청소 회망 날자", options:['협의 가능해요','가능한 빨리 진행하고 싶습니다','일주일 이내로 진행하고 싶습니다','원하는 날짜가 있습니다','기타']},
      {id: 10.2, text: "주소를 선택해 주세요",options: [
        { label: "서울", districts: ["강남구", "강동구", "강북구", "강서구", "관악구", "광진구", "구로구", "금천구", "노원구", "도봉구", "동대문구", "동작구", "마포구", "서대문구", "서초구", "성동구", "성북구", "송파구", "양천구", "영등포구", "용산구", "은평구", "종로구", "중구", "중랑구"] },
        { label: "인천", districts: ["강화군", "계양구", "남동구", "동구", "미추홀구", "부평구", "서구", "연수구", "옹진군"] },
      ...
      if(!authToken){
        ...
        try {
          ...
          const response = await axios.post(`${urlApi}/auth/interior`,
            { answeredQuestions, estimateType: '청소' }, {
              headers: {
  ...
  const handleNextQuestion = () => {
    ...
    if (currentQuestionSet === questionSets[0]) {
      ...
      if (selectedOption === "에어컨 청소") {
        setCurrentSetIndex(2);
        setIsOptionSelected(false);
      } else if (selectedOption === "이사 청소" ||"입주 청소") {
