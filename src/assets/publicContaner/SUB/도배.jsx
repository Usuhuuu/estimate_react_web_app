...
const FloorandWall = () => {
  ...
  [ 
    {id:5.1, text:"선택 해 주세요",options:['도배', '장판', '도배+ 장판'] },
  ],
  [
    {id:5.2, text:"집 시공 원하는 곳",options:['집 전체','거실','주방','방','천장','기타'] },
    {id:5.3, text:"어떤 재질 벽지를 원하는지",options:['필요 없음','싱담 후 결정', '전체 실크', '전체 합치','실크+합치','도배지 보유', '기타'] },
    {id:5.4, text:"어떤 시공을 원하는지",options:['필요 없음','싱담 후 결정','전체 교체(철거 후 시공)','부분 교체(철거 후 결정)', '신규 시공','기타' ] },
      ...
      if(!authToken){
        ...
        try {
          ...
          const response = await axios.post(`${urlApi}/auth/interior`,
            { answeredQuestions, estimateType: '도배' }, {
              headers: {
